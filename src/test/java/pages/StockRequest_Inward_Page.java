...
public class StockRequest_Inward_Page extends TestBase {
	...
	private static WebElement Inventory_Inward_Cancel_Cancel;
	
	@FindBy(xpath = "//span[contains(text(),'Yes')]")
	private static WebElement Inventory_Inward_Cancel_Request_Yes_Button;
...
//		CommonMethod.clickonWebElement(Inventory_Filter_Outside, "Outside to avoid element overlaping");
		...
		test.pass("Global Search Key is working fine for requestID");
		...
		String delivery_to = driver.findElement(By.xpath("//tbody/tr[1]/td[8]")).getText();
		Assert.assertNotEquals(delivery_to, "NA");
		test.pass("filter and Global Search Key are working fine ");

		return this;
	}

	public StockRequest_Inward_Page stock_request_Create_Valid_Inward_With_Manual_entry_of_SKU(
...
//		
		...
		CommonMethod.inputCalenderDate(Inventory_Inward_Quantity1_Field,
				ht.get("Quantity1 to enter manually to inward"));
		CommonMethod.inputCalenderDate(Inventory_Inward_Quantity2_Field,
				ht.get("Quantity2 to enter manually to inward"));
...
//			((JavascriptExecutor) driver).executeScript("arguments[0].click();", Inventory_Inward_Action_Edit);
		...
		((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView(true);",
				Inventory_Inward_Mark_As_Received);
		Thread.sleep(3000);
...
//		Inventory_Inward_Stock_Request_Bredcrump.click();
		...
		String Status = Inventory_Inward_Table_Status.getText();

		Assert.assertEquals(Status, "RECEIVED", "Status is not Received");
		wait.until(ExpectedConditions.invisibilityOfElementLocated(By.className("spinner-wrapper")));

		((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView(true);", Inventory_Inward_ItemDetails);
		Thread.sleep(3000);
		...
		CommonMethod.clickonWebElement(Inventory_Inward_SKU1_from_Item_Details_Table,
				"Inventory_Inward_SKU1_from_Item_Details_Table Button");
	...
	public StockRequest_Inward_Page stock_request_Create_Valid_Inward_using_Product_File(Hashtable<String, String> ht)
...
//	Inventory_Inward_Stock_Request_Bredcrump.click();
	...
	public StockRequest_Inward_Page stock_request_Inward_Cancel_Request(Hashtable<String, String> ht)
			throws InterruptedException {
		WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(30));
		wait.until(ExpectedConditions.invisibilityOfElementLocated(By.className("spinner-wrapper")));
...
//		((JavascriptExecutor) driver).executeScript("arguments[0].click();", Inventory_Inward_Action_Edit);
		...
		CommonMethod.clickonWebElement(Inventory_Inward_Download_Create_Request_Button,
				"Download Stock Request Button");
		CommonMethod.clickonWebElement(Inventory_Inward_Cancel_Request_Button, "Cancel Stock Request Button");
		
		CommonMethod.clickonWebElement(Inventory_Inward_Cancel_Request_Yes_Button, "Cancel Stock Request Button");
		
		wait.until(ExpectedConditions.invisibilityOfElementLocated(By.className("spinner-wrapper")));

		String Status = Inventory_Inward_Table_Status.getText();

		Assert.assertEquals(Status, "CANCELED", "Status is not Received");
	...
	public StockRequest_Inward_Page stock_request_Inward_Error_Message_Validation(Hashtable<String, String> ht)
			throws InterruptedException {
		WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(30));
		wait.until(ExpectedConditions.invisibilityOfElementLocated(By.className("spinner-wrapper")));
...
//		((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView(true);", Inventory_Inward_Approve);
	...
	public DashBoardPages Breadcrump(Hashtable<String, String> ht) throws InterruptedException, AWTException {
		...
		wait.until(ExpectedConditions.invisibilityOfElementLocated(By.className("spinner-wrapper")));
		((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView(true);",
				Inventory_Inward_Stock_Request_Bredcrump);
		Thread.sleep(3000);
		Inventory_Inward_Stock_Request_Bredcrump.click();
